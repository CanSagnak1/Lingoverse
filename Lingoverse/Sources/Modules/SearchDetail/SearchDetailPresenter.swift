...
enum SearchDetailState: Equatable {
    case loading
    case content(
        header: SearchDetailHeaderVM, segments: SearchDetailSegmentVM,
        meanings: [SearchDetailMeaningVM])
    case error(String)
}
...
final class SearchDetailPresenter: SearchDetailViewOutput, SearchDetailInteractorOutput {
    ...
    func didLoadContent(word: WKWord) {
        ...
        let segmentTitles = word.meanings.map { Strings.localizedPartOfSpeech($0.partOfSpeech) }
        ...
        let meaningsVM: [SearchDetailMeaningVM] = word.meanings.map { meaning in
            let definitionsVM: [SearchDetailDefinitionVM] = meaning.definitions.enumerated().map {
                (index, def) in
            ...
            return SearchDetailMeaningVM(
                partOfSpeech: Strings.localizedPartOfSpeech(meaning.partOfSpeech),
